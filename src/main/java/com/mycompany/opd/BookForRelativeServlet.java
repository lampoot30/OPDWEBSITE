...
public class BookForRelativeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            String servicesSql;
            if (age != -1 && age <= 17) {
                // If age is 17 or below, only show Pediatric Medicine
                servicesSql = "SELECT * FROM services WHERE service_name = 'Pediatric Medicine'";
            } else {
                servicesSql = "SELECT * FROM services WHERE service_name IN ('Family Medicine', 'Women''s Health Clinic', 'Occupational Therapy', 'Rehabilitation Medicine') ORDER BY service_name ASC";
            }
