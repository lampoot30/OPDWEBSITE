...
public class UpdateDoctorProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            StringBuilder profileSql = new StringBuilder("UPDATE user_profiles SET surname=?, given_name=?, middle_name=?, date_of_birth=?, gender=?, permanent_address=?, specialization=?, license_number=?, license_expiry_date=?");
            ...
            try (PreparedStatement psProfile = conn.prepareStatement(profileSql.toString())) {
                ...
                psProfile.setString(7, request.getParameter("specialization"));
                psProfile.setString(8, request.getParameter("licenseNumber"));
                psProfile.setDate(9, Date.valueOf(request.getParameter("licenseExpiryDate")));
    ...
    private void storeProfileDataInSession(HttpSession session, HttpServletRequest request, String newContactNumber, String newPassword, String profilePicPath) {
        ...
        profileData.put("permanent_address", request.getParameter("permanent_address"));
