...
public class UpdateAdminProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isPasswordChanging) {
            if (newPassword == null || newPassword.isEmpty() || !newPassword.equals(confirmPassword)) {
                response.sendRedirect("edit-admin-profile?error=New passwords do not match or are empty.");
                return;
            }
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        if (PasswordUtil.verifyPassword(currentPassword, rs.getString("password"))) {
                            ...
                            response.sendRedirect("edit-admin-profile?error=Current password is incorrect.");
                            return;
                        }
                    }
                }
            } catch (SQLException e) {
                response.sendRedirect("edit-admin-profile?error=Database error verifying password.");
                return;
            }
        }
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            e.printStackTrace();
            response.sendRedirect("admin-profile?error=Could not verify current contact number.");
            return;
        }
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            StringBuilder profileSql = new StringBuilder("UPDATE user_profiles SET surname=?, given_name=?, middle_name=?, date_of_birth=?, gender=?, permanent_address=?");
            ...
            try (PreparedStatement psProfile = conn.prepareStatement(profileSql.toString())) {
                ...
                psProfile.setString(6, request.getParameter("address"));
            ...
            String redirectURL;
            if ("Super Admin".equals(userType)) {
                redirectURL = "super-admin-profile";
            } else {
                redirectURL = "admin-profile";
            }

            AuditLogger.log(userId, username, (String) session.getAttribute("userType"), "Updated own profile information");
    ...
    private void storeProfileDataInSession(HttpSession session, HttpServletRequest request, String newContactNumber, String newPassword, String profilePicPath) {
        ...
        profileData.put("address", request.getParameter("address"));
