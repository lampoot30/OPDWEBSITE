...
public class StaffAddAppointmentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            // Fetch services that are "by schedule" or "by appointment"
            String servicesSql = "SELECT * FROM services WHERE schedule_details LIKE '%by schedule%' OR schedule_details LIKE '%by appointment%' OR service_name = 'Family Medicine' ORDER BY service_name ASC";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            String sql = "INSERT INTO appointments (booked_by_user_id, relative_id, patient_type, last_name, given_name, middle_name, birthday, civil_status, contact_number, reason_for_visit, preferred_date, status, assigned_doctor_id, assigned_room_id) " +
                         "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Accepted', ?, ?)";
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ps.setInt(12, Integer.parseInt(request.getParameter("doctorUserId")));

                String roomIdStr = request.getParameter("roomId");
                if (roomIdStr != null && !roomIdStr.isEmpty()) {
                    ps.setInt(13, Integer.parseInt(roomIdStr));
                } else {
