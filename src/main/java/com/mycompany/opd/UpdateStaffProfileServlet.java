    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isPasswordChanging) {
            if (newPassword == null || newPassword.isEmpty() || !newPassword.equals(confirmPassword)) {
                response.sendRedirect("edit-staff-profile?error=New passwords do not match or are empty.");
                return;
            }
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        if (PasswordUtil.verifyPassword(currentPassword, rs.getString("password"))) {
                            ...
                            response.sendRedirect("edit-staff-profile?error=Current password is incorrect.");
                            return;
                        }
                    }
                }
            } catch (SQLException e) {
                response.sendRedirect("edit-staff-profile?error=Database error verifying password.");
                return;
            }
        }
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            e.printStackTrace();
            response.sendRedirect("staff-profile?error=Could not verify current contact number.");
            return;
        }
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            AuditLogger.log(userId, username, "Staff", "Updated own profile information");
            response.sendRedirect("staff-profile?success=Profile updated successfully!");

        } catch (SQLException e) {
            e.printStackTrace();
            response.sendRedirect("edit-staff-profile?error=Database error: " + e.getMessage());
        }
    }
