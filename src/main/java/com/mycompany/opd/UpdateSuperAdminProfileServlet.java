...
public class UpdateSuperAdminProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            if (profileExists) {
                profileSql = "UPDATE user_profiles SET surname=?, given_name=?, middle_name=?, date_of_birth=?, gender=?, permanent_address=?" + (profilePicPath != null ? ", profile_picture_path=?" : "") + " WHERE user_id=?";
            } else {
            ...
            try (PreparedStatement ps = conn.prepareStatement(profileSql)) {
                ...
                int paramIndex = 7;
                if (profilePicPath != null) ps.setString(paramIndex++, profilePicPath);
                if (!profileExists && profilePicPath == null) ps.setNull(paramIndex++, java.sql.Types.VARCHAR); // For INSERT
